...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/17 17:01:24 by sjadalla          #+#    #+#             */
/*   Updated: 2022/04/19 16:29:35 by sjadalla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd > OPEN_MAX)
		return (NULL);
	static_var[fd] = ft_read(fd, static_var[fd]);
	if (!static_var[fd])
		return (NULL);
	next_line = ft_get_line (static_var[fd]);
	static_var[fd] = ft_save(static_var[fd]);
	return (next_line);
}
